...
const slides = [
  {
    ...
    title: 'Karachi Fintech App: UX Results That Drive Growth',
    bullets: [
      'Challenge: 71% drop-off in onboarding',
      'Solution: Mobile UI UX design + simplified flows',
      'Result: 92% completion rate, 3.8x faster onboarding'
    ],
    image: '/assets/img/ui-ux-design/fintech-app-ux.png'
  },
  {
    category: 'E-commerce Redesign',
    title: 'Pakistan E-commerce: UX Results That Drive Growth',
    bullets: [
      'Challenge: 43% cart abandonment',
      'Solution: Website redesign + checkout optimization',
      'Result: 68% conversion lift, 247% engagement growth'
    ],
    image: '/assets/img/ui-ux-design/ecommerce-redesign-ux.png'
  }
]

export default function UiUxShowcaseSlider() {
  ...
  return (
    <div className="spotlight-card position-relative overflow-hidden" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} role="region" aria-label="UI/UX showcase carousel">
      {slides.map((s, i) => (
        <div key={i} className={`slide-item ${i === index ? 'active' : 'inactive'}`} aria-hidden={i !== index}>
          <div className="row g-0 align-items-stretch">
            <div className="col-lg-6 p-xxl-15 p-xl-12 p-lg-10 p-6 d-flex flex-column justify-content-center bg-white-opacity" style={{ minHeight: '600px' }}>
              ...
              <h2 className="white-clr display-6 fw-bold mb-6 lh-sm">{s.title}</h2>
